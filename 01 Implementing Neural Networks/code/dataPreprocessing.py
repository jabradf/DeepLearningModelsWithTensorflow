...
my_ct = ColumnTransformer(
  [('scale', StandardScaler(),['age','bmi','children'])], remainder='passthrough')

#fit the scaler to the training data
features_train_scale = my_ct.fit_transform(features_train)
#applied the trained scaler on the test data
features_test_scale = my_ct.transform(features_test)

# convert the transformed data to PD dataframe
features_train_scale = pd.DataFrame(features_train_scale, columns = features_train.columns)
features_test_scale = pd.DataFrame(features_test_scale, columns = features_test.columns)

print("features_train_scale stats:", features_train_scale.describe())
print("features_test_scale stats:", features_test_scale.describe())
